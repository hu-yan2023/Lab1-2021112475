...
public class GraphProcessor {
	...
	public void generateGraphFromFile(String filePath) throws IOException {
	    ...
	    String[] word = content.toString().replaceAll("[^a-zA-Z ]", " ")
	      .toLowerCase().split("\\s+"); // 使用空格分割单词
	  ...
	  private String generateGraphText() {
	    ...
	    for (String node : graph.keySet()) {
	      ...
	      for (Map.Entry<String, Integer> neighbor : neighbors.entrySet()) {
	        graphTextBuilder.append("  ").append(node).append(" -> ").append(neighbor.getKey())
	          .append(" [label=").append(neighbor.getValue()).append("];\n");
	      }
	    }
	  ...
	  public void calcShortestPath(String word1, String word2) {
	    ...
	    if ("!".equals(word2)) {
	      for (String word : nodes) {
	        ...
	        System.out.print("The shortest path from " + word1 + " to " 
	            + word + " : length " + dis.get(word) + " " + word);
